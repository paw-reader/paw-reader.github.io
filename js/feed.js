...
import { formatBytes, getServiceColor, getMediaUrl, showMediaUnavailableWarning, startProgress, stopProgress, getServiceCreatorUrl, getServicePostUrl } from './utils.js';
...
export const playbackObserver = new IntersectionObserver((entries) => {
...
export const feedObserver = new IntersectionObserver((entries) => {
...
export async function loadMediaWithProgress(item) {
  ...
  if (type === 'zip') {
    ...
    btnAbort.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px;"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="9" y1="9" x2="15" y2="15"></line><line x1="15" y1="9" x2="9" y2="15"></line></svg> Abort';
    ...
    btnPause.addEventListener('click', (e) => {
      e.stopPropagation();
      isPaused = !isPaused;
      btnPause.innerHTML = isPaused ? `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px;"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg> Resume` : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px;"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg> Pause`;
    });
  ...
  if (type === 'video' || type === 'audio') {
    ...
    video.addEventListener('error', () => {
      ...
      if (path && (state.currentSite === 'pawchive' || state.currentSite === 'kemono')) {
        let thumbUrl = state.currentSite === 'pawchive'
  ...
  try {
    ...
    img.onload = () => { if (progressOverlay) progressOverlay.style.display = 'none'; };
...
export function createPostCard(post) {
  ...
  if (!hasAvailableMedia && settingHideNoMedia && settingHideNoMedia.checked) {
    if (!state.currentFeedEndpoint.includes('/announcements') && !state.currentFeedEndpoint.includes('/dms') && !state.currentFeedEndpoint.includes('/fancards')) {
      return null;
    }
  }

  if (state.currentSite === 'cum' && state.cumSelectedTypes && state.cumSelectedTypes.length > 0 && state.cumSelectedTypes.length < 4) {
    const isTextPost = allMedia.length === 0;
    if (isTextPost) {
      if (!state.cumSelectedTypes.includes('text') && !state.currentFeedEndpoint.includes('/announcements') && !state.currentFeedEndpoint.includes('/dms') && !state.currentFeedEndpoint.includes('/fancards')) {
        return null;
      }
    } else {
      const hasVideo = allMedia.some(m => ['mp4', 'webm', 'mov'].includes(m.path.split('.').pop().toLowerCase()));
      const hasAudio = allMedia.some(m => ['mp3', 'ogg', 'wav', 'm4a'].includes(m.path.split('.').pop().toLowerCase()));
      const hasImage = allMedia.some(m => ['jpg', 'jpeg', 'png', 'gif', 'webp', 'avif', 'svg'].includes(m.path.split('.').pop().toLowerCase()));
  ...
  const hasCJK = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\uac00-\ud7af]/.test(displayName || '');
...
export async function fetchPosts() {
  ...
  try {
    ...
    const url = isSinglePageFeed ? state.currentFeedEndpoint : `${state.currentFeedEndpoint}${separator}o=${state.offset}`;
