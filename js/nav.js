...
export function updateNavVisibility(mouseY = window.lastMouseY) {
  ...
  const dropdownOpen = !!document.getElementById('linked-accounts-dropdown') || !!document.getElementById('cum-posts-dropdown');
...
export function updateSiteSpecificUI() {
  ...
  if (sortSelect) {
    ...
    if (state.currentSite === 'cum') {
      ...
      `;
      sortSelect.value = ['popularity', 'indexed', 'updated', 'alphabetical', 'service', 'dms', 'posts'].includes(prevVal) ? prevVal : 'popularity';
    } else {
      sortSelect.innerHTML = `
        <option value="popularity">Popularity</option>
        <option value="indexed">Date Indexed</option>
      ...
      `;
      sortSelect.value = ['popularity', 'indexed', 'updated', 'alphabetical', 'service'].includes(prevVal) ? prevVal : 'popularity';
    }
  }
...
export function updateNavTabs(creator) {
  ...
  if (state.currentSite === 'kemono' || state.currentSite === 'pawchive') {
    ...
    const isFansly = (creator.service || '').toLowerCase() === 'fansly' || (creator.allPlatforms && creator.allPlatforms.some(p => (p.service || '').toLowerCase() === 'fansly'));
    if (isFansly) {
  ...
  tabs.forEach((tab, index) => {
    ...
    if (state.currentSite === 'cum' && tab === 'Posts') {
      if (creator.imageCount > 0) {
        postCategories.push({ key: 'photos', label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px;vertical-align:-2px;"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg> Photos' });
      }
      if (creator.videoCount > 0) {
        postCategories.push({ key: 'videos', label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px;vertical-align:-2px;"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg> Videos' });
      }
      if (creator.audioCount > 0) {
        postCategories.push({ key: 'audio', label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px;vertical-align:-2px;"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg> Audio' });
      }
      if (creator.postCount > 0) {
        postCategories.push({ key: 'text', label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px;vertical-align:-2px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg> Text' });
      }
    }
    ...
    if ((state.currentSite === 'kemono' || state.currentSite === 'pawchive') && (tab === 'DMs' || tab === 'Announcements' || tab === 'Fancards')) {
